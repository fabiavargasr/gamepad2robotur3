...
def modelo_calibracion(joint_angles):
   

#    joint_angles = np.round(joint_angles, decimals=3) # in radians

    joints_pos2=np.asarray(joint_angles)-[np.pi,0,0,0,0,0]
#    joints_pos2=np.asarray(joints_pos)+[0,0,0,0,0,0]

    print("real articulaciones 2")
    print(joint_angles) 
    ...
    print("PPosicion actual:")
    print(ee_pose)
   
    ee_pose[0,3]=ee_pose[0,3]+0.010
    ee_pose[1,3]=ee_pose[1,3]+0.015
   # ee_pose[2,3]=ee_pose[2,3]+0.015    
    
    
    print("nueva pose:")
 
   # joints_pos3=np.asarray(joints_pos2)+[np.pi,0,0,0,0,0]
   ...
   # print("\nTesting inverse kinematics:\n")
    ...
    print("salida de inversa:")
    
    
    print(joint_configs)
 
    
    #probamos enviar el mismo valor que llego
...
#    
    ...
    for joint_config in joint_configs:
     ...
     #   print("hola")
        index_ar.append(np.sum(np.abs(joint_config-np.asarray(joints_pos2))))
    index_min = min(range(len(index_ar)), key=index_ar.__getitem__)
    
    print "articulacion a enviar 2222"

    joint1=joint_configs[index_min]
    
    joint1=joint1 +[np.pi,0,0,0,0,0]
    
    print(np.rad2deg(joint1))
...
def calibracion_ursim():  #recibe cuando hay cambio en la posicion del joystic, en ese caso, se consulta en que modo se encuentra para estableceer un plan de trabajo respectivo a cada modo.
#msg=[x,y,z,rx,ry,rz]
...
#             rospy.init_node("test_move", anonymous=True, disable_signals=True)
       ...
       #move1()
        ...
        try:
            ...
            Q1=modelo_calibracion(joints_pos)
    #        print Q1
