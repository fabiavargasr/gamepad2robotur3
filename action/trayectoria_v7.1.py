...
def modelo_calibracion(joint_angles):
    ...
    ee_pose[0,3]=ee_pose[0,3]+var2_x/2
    ee_pose[1,3]=ee_pose[1,3]+var2_y/2
    ee_pose[2,3]=ee_pose[2,3]+var2_z/2    
    print("nueva pose:")
...
def modelo_traj_v2(joint_angles,x,y,z):
...
#             rospy.init_node("test_move", anonymous=True, disable_signals=True)
       ...
       #move1()
        ...
        try:
            joint_states = rospy.wait_for_message("joint_states", JointState)
            joints_pos = joint_states.position
            Q1=modelo_traj_v2(joints_pos,xx,yy,zz)
...
def main():
    ...
    try:
        ...
        # calibracion_ursim()
        for n1 in len(path_ar[0]):
            print ni
            print ("ciclo", path_ar[ni])
